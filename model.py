...
def _morph_unit(inputs, filters=4, kernel=(3, 3)):
  erosion = Erosion2D(filters, kernel,padding="same",strides=(1,1), kernel_initializer='he_normal')(inputs)
  dilation = Dilation2D(filters, kernel,padding="same",strides=(1,1), kernel_initializer='he_normal')(inputs)
  xc = Concatenate()([erosion, dilation])
  xc = Conv2D(2*filters,kernel,padding='same',activation='linear',kernel_initializer='he_normal')(xc)
  return xc


def _bn_relu_conv_block(inputs, filters, kernel=(3, 3), stride=(1, 1)):
  x = Conv2D(filters, kernel, padding='same', use_bias=False, kernel_initializer='he_normal',strides=stride)(inputs)
...
def _dual_path_block(input, pointwise_filters_a, grouped_conv_filters_b, pointwise_filters_c, filter_increment, cardinality, block_type='normal'):
  ...
  if projection:
    projection_path = _bn_relu_conv_block(inputs, filters=pointwise_filters_c + 2 * filter_increment, kernel=(1, 1), stride=stride)
  ...
  x = _grouped_morph_block(x, grouped_channels=grouped_channels, cardinality=cardinality)
...
def _decoder_block(inputs, skip_connection, filters):
  upsampled_input = Conv2DTranspose(filters = int(ip.shape[-1]), kernel_size =(3,3), strides = (2,2), padding = 'same' )(inputs)
...
def _morph_block(inputs, num_filters, filter_increment = 12, cardinality=6):
  x = inputs
  x = _dual_path_block(x, num_filters, num_filters, num_filters, filter_increment, cardinality, 'projection')
  x = _dual_path_block(x, num_filters, num_filters, num_filters, filter_increment, cardinality, 'normal')
